Cover.prototype.encode = function(message, image, options) {
  ...
  exports.secretenkripsi = async (req, res) => {
  try {
    ...
    // Kode steganografi di sini
    const message = "Pesan Rahasia Anda";
    const encodedImage = util.encode(message, uploadedImagePath, { /* tambahkan opsi jika diperlukan */ });

    // Simpan gambar terenkripsi ke disk
    fs.writeFileSync(uploadedImagePath, encodedImage, 'binary');

    const insertgambar = 'INSERT INTO enkripsi (idusername, type, value) VALUES (?, ?, ?)';
    connection.query(insertgambar, [sessionId, "Image Encryption", uploadedImagePath], (error, results) => {
      if (error) {
